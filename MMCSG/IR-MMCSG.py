...
class MultimodalBartForConditionalGeneration(BartPretrainedModel):
    ...
    def forward(
        ...
        audio_input=None,
        ppc_input=None,
        intent_labels = None,      # New addition        
...
class MultimodalBartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.fusionVis_at_layer = [4]
        self.fusionAud_at_layer = [4]
        self.fusionPpc_at_layer = [3]
        self.visual_transformer = TransformerEncoder(d_model=VISUAL_DIM, 
                                                     num_layers=4,
                                                     num_heads=8, 
                                                     dim_feedforward=VISUAL_DIM)
        ...
        self.MAF_layer = MAF(dim_model=embed_dim,
                             dropout_rate=0.2)
        self.MAF_layer_A = MAF_A(dim_model=embed_dim,
                             dropout_rate=0.2)
        self.MAF_layer_P = MAF_P(dim_model=embed_dim,
                             dropout_rate=0.2)
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if idx in self.fusionAud_at_layer:
                audio_input = self.audio_transformer(audio_input)[-1]
                hidden_states = self.MAF_layer_A(text_input=hidden_states,
                                               audio_context=audio_input)
                                               #visual_context=visual_input)

            if idx in self.fusionPpc_at_layer:
                ppc_input = self.ppc_transformer(ppc_input)[-1]
                hidden_states = self.MAF_layer_P(text_input=hidden_states,
...
def train(model,
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        print("\ntest_rouge_1: {}\ttest_rouge_2: {}\ttest_rouge_L: {}\ttest_bleu_1: {}\ttest_bleu_2: {}\ttest_bleu_3: {}\ttest_bleu_4: {}\ttest_meteor: {}\ttest_BS_1: {}\test_JS: {}\tCR_Acc: {}\tCR_f1:{}".format(test_results['rouge_1'], test_results['rouge_2'], test_results['rouge_L'], test_results['bleu_1'], test_results['bleu_2'], test_results['bleu_3'], test_results['bleu_4'], test_results['meteor'],test_results['BS'], test_results['JS'],test_results['CR_Acc'],test_results['CR_f1'] ))
...
def get_val_scores(model,
                   ...
                   epoch,
                   **gen_kwargs):
    predictions, gold, CR = test_epoch(model,
                                   tokenizer,
                                   data_loader,
                                   desc=desc,
                                   **gen_kwargs)
    ...
    if "Validation" in desc and epoch == MAX_EPOCHS - 1:
        ...
        file_name = "./M-MDS_TKV" + str(epoch+1) + "_val_results.csv"
...
def test_epoch(model,
    ...
    with torch.no_grad():
        for step, batch in enumerate(tqdm(data_loader, desc=desc)):
            ...
            encoder_last_hidden_state_clone = outputs['encoder_last_hidden_state'].detach().clone()
            logits_intent = model2(encoder_last_hidden_state_clone)

            intent_prediction = logits_intent.argmax(dim = -1).tolist()
...
def prepare_for_training(model,
    ...
    optimizer = AdamW(
        [
            ...
            # {'params': new_params_list2,'lr': CLS_LR, 'weight_decay': weight_decay}           
        ],
    ...
    del new_params_list
    del new_params_list2
    gc.collect()
    torch.cuda.empty_cache()
   
    return optimizer

class ClassifyIntent(torch.nn.Module):
    def __init__(self):
        super(ClassifyIntent, self).__init__()
    ...
    def forward(self, encoder_hidden_representation):
            
        temp_value = self.classifier(encoder_hidden_representation.mean(dim = 1))
        # outputs.encoder_last_hidden_state.mean(dim = 1)
        return temp_value

model2 = ClassifyIntent()
model2 = model2.to(DEVICE)    

optimizer_intent = AdamW(
                        model2.parameters(),
        lr=1e-3)
...
def train_epoch(model,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc="Training Iteration")):
        ...
        loss = outputs['loss']
        encoder_last_hidden_state_clone = outputs['encoder_last_hidden_state'].detach().clone()
        # encoder_last_hidden_state_clone = outputs['encoder_last_hidden_state']
        logits_intent = model2(encoder_last_hidden_state_clone)
        
        loss_function = CrossEntropyLoss()
        intent_loss = loss_function(logits_intent, intent_labels)
    ...
    # return epoch_train_loss/ step , accuracy
...
Dataset = Dataset.astype({"encoded_merged_intent": int})
print(set(Dataset['encoded_merged_intent']))
...
# print(train_data['Group_mapped'].value_counts())

#test_data.to_csv("/home/abhisek_1921cs16/New/MMCSG/Multimodal_summary_5/Test_results_T_I_ODS/test_data.csv")
