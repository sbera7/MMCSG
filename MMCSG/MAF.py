...
TEXT_INPUT_PATH = './OurData/Text/'
ACOUSTIC_INPUT_PATH = './OurData/Audio/'
VISUAL_INPUT_PATH = './OurData/Video/'
...
class MAF(nn.Module):
    ...
    def __init__(self,
        ...
        # self.PPCC_transform = nn.Linear(PPCC_MAX_LEN, SOURCE_MAX_LEN, bias=False) # PPCC
        ...
        # PPCC Attention
        # self.PPCC_attention = ContextAwareAttention(dim_model=dim_model,
        ...
        # self.PPCC_gate = nn.Linear(2*dim_model,dim_model)
    ...
    def forward(self,
                ...
                PPCC:Optional[torch.Tensor]=None):
        ...
        # # Personality Info context for attention
        # PPCC = PPCC.permute(0, 2, 1)
        # PPCC = self.PPCC_transform(PPCC)
        # PPCC = PPCC.permute(0, 2, 1)
        
        # ppcc_out = self.PPCC_attention(q=text_input,
        #                                           k=text_input,
        #                                           v=text_input,
        #                                           context=PPCC)
        ...
        # weight_p = F.sigmoid(self.PPCC_gate(torch.cat((ppcc_out, text_input), dim=-1)))
...
class MultimodalBartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        # self.ppcc_transformer = TransformerEncoder(d_model=PPCC_DIM, 
        #                                                num_layers=4,
        #                                                num_heads=2, 
    ...
    def forward(
        ...
        # ppcc_input=None,        # New addition of ppcc_input
        head_mask=None,
        inputs_embeds=None,
...
class MultimodalBartForConditionalGeneration(BartPretrainedModel):
    ...
    def forward(
        ...
        outputs = self.model(
            ...
            acoustic_input=acoustic_input,      # New addition of acoustic_input
            visual_input=visual_input,      # New addition of visual_input
            # ppcc_input = ppcc_input,               # New addition of ppcc_input
...
def preprocess_dataset(dataset):
    ...
    model_inputs['acoustic_input'] = torch.tensor(dataset["audio_features"]).to(DEVICE)
...
def train_epoch(model,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc="Training Iteration")):
        ...
        optimizer.zero_grad()
        acoustic_input = acoustic_input.unsqueeze(dim = 1)
...
def test_epoch(model,
    ...
    with torch.no_grad():
        for step, batch in enumerate(tqdm(data_loader, desc=desc)):
            ...
            decoded_contxt = tokenizer.batch_decode(input_ids, skip_special_tokens=True)
                
            predictions.extend(decoded_preds)
            gold.extend(decoded_labels)
            contxt.extend(decoded_contxt)
    ...
    return predictions, gold , contxt
...
def get_val_scores(model,
                   ...
                   epoch,
                   **gen_kwargs):
    predictions, gold , contxt = test_epoch(model,
                                   tokenizer,
                                   data_loader,
                                   desc=desc,
                                   **gen_kwargs)
    ...
    if "Validation" in desc:
        ...
        file_name = RESULT_OUTPUT_DIR + "val/Multimodal_mmcs" + str(epoch+1) + "_val_results.csv"
        val_df.to_csv(file_name, index=False) 
        print("Validation File saved")
        
    elif "Test" in desc:
        test_df = pd.DataFrame(list(zip(contxt, gold, predictions)), columns=['context','actual_explanation', 'predicted_explanation'])
        file_name = RESULT_OUTPUT_DIR + "test/Multimodal_mmcs" + str(epoch+1) + "_test_results.csv"
...
if __name__ == "__main__":
    ...
    NewDataset_path = './OurData/mmcs_final_with_pf_new.json'
